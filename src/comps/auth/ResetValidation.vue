              ...
              <div>
                <span>
                  <span class="field-container">
                    ...
                    <div class="field">
                        <label class="label login-border">Nom</label>
                        <div class="control has-icons-right">
                        <input v-model="name" type="text" placeholder="Votre nom" required class="input is-success">
                        </div>
                    </div>
                    ...
                    <div class="field">
                        <label class="label login-border">Nouveau mot de passe</label>
                        <div class="control has-icons-right">
                        <input v-model="password" type="password" placeholder="Votre nouveau mot de passe" required class="input is-success">
                        </div>
                    </div>

                    <!-- Confirm Password input -->
                    <div class="field">
                        <label class="label login-border">Confirmation du mot de passe</label>
                        <div class="control has-icons-right">
                        <input v-model="confirmPassword" type="password" placeholder="Confirmation du mot de passe" required class="input is-success">
                        </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    getInfo: function () {
      ...
      if (this.db && this.token) {
        this.$axios.get(process.env.ODOO_API + 'api/reset_password?db=' + this.db + '&token=' + this.token, { headers }).then(response => {
            console.log(response)
            if (response.data.error && response.data.error.length > 1) {
                if (response.data.invalid_token === true) {
                    this.$toastr.Add({
                        name: 'ErrorCredentials',
                        title: 'Réinitialisation de mot de passe',
                        msg: 'La session d\'authentification est invalide',
                        clickClose: true,
                        timeout: 3000,
                        progressBarValue: 0,
                    ...
                    this.$toastr.Add({
                        name: 'ErrorCredentials',
                        title: 'Réinitialisation de mot de passe',
                        msg: 'Une erreur est survenu, veuillez réessayer ultérieurement !',
                        clickClose: true,
                        timeout: 3000,
                        progressBarValue: 0,
                        position: 'toast-top-right',
    ...
    reset: function () {
      ...
      const headers = { 'Content-Type': 'application/x-www-form-urlencoded' }
      if (this.db && this.token) {
        this.$axios.post(process.env.ODOO_API + 'api/reset_password?db=' + this.db + '&token=' + this.token, formData, { headers }).then(response => {
            if (response.data.status === 200) {
                this.$toastr.Add({
                    name: 'SuccessPasswordChanged',
                    title: 'Réinitialisation de mot de passe',
                    msg: 'Votre mot de passe a bien été changé, vous pouvez désormais vous connecter.',
                    clickClose: true,
                    timeout: 3000,
                    progressBarValue: 0,
                    position: 'toast-top-right',
