...
export default {
  ...
  async created () {
    ...
    this.clients = await this.$axios.get(process.env.ODOO_API + 'api/v2/partners?session_id=' + this.user.session_id, { headers: header }).then(response => {
  ...
  methods: {
    ...
    async getSiretData(name) {
      ...
      if (/^\d{9}$/.test(cleanedSiret) || /^\d{14}$/.test(cleanedSiret)) {
        const queryType = cleanedSiret.length === 14 ? 'siret' : 'siren';
        
        await this.$axios.get('https://data.opendatasoft.com/api/records/1.0/search/', {
          params: {
            dataset: 'economicref-france-sirene-v3@public',
            q: `${queryType}:${cleanedSiret}`,
            rows: 1
          }
        }).then(async response => {
          ...
          if (resJson.records && resJson.records.length > 0) {
            ...
            const city = rawRecord.libellecommuneetablissement || '';
            let country = 'France';

            if (city) {
              try {
                const geoResponse = await this.$axios.get('https://nominatim.openstreetmap.org/search', {
                  params: {
                    q: city,
                    format: 'json',
                    addressdetails: 1,
                ...
                if (geoResponse.data.length > 0) {
                  country = geoResponse.data[0].address.country || 'France';
                }
              } catch (error) {
      ...
      if (this.siretData) {
        ...
        let params = { 'siret_data': [this.siretData] };
        let newclient = await this.$axios.post(
          process.env.ODOO_API + 'api/v2/create_client?session_id=' + this.user.session_id,
          { params },
          { headers: header }
        ...
        this.$toastr.Add({
          ...
          msg: 'Veuillez vérifier le SIREN ou le SIRET du client',
          clickClose: true,
          timeout: 3000,
          progressBarValue: 0,
          position: 'toast-top-right',
    ...
    async meClient () {
      ...
      let meclient = await this.$axios.post(process.env.ODOO_API + 'api/v2/my_client?session_id=' + this.user.session_id, { headers: header }).then(response => {
    ...
    selectClient (c) {
      this.client = {
        ...
        siret: c.siret,
        tel: c.tel,
        address: `${c.street} ${c.zip} ${c.city} ${c.country}`,
        zip: c.zip,
        city: c.city
      }
