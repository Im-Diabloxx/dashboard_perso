    ...
    <div class="container is-max-widescreen">
      <div class="columns is-desktop mt-1" style="padding-right: 0.75rem">
        ...
        <div class="column is-three-fifths-desktop" style="padding: 0.75rem; padding-right: 0rem;">
           <div class="fieldset">
            <label>Où est mon IPBX</label>
            <div class="fieldset-content">
              <div
                v-if="zabbix_datas && zabbix_datas['gps_latitude'] && zabbix_datas['gps_longitude']"
                class="map-container"
              >
                <l-map
                  ref="map"
                  :zoom="zoom"
                  :center="[zabbix_datas['gps_latitude'], zabbix_datas['gps_longitude']]"
                  :options="{ zoomControl: true, dragging: true, scrollWheelZoom: true }"
                  class="leaflet-map"
                >
                  <l-tile-layer
                  ...
                  />
                  <l-marker
                    :lat-lng="[zabbix_datas['gps_latitude'], zabbix_datas['gps_longitude']]"
                    :icon="customIcon"
                  >
                    <l-popup>
                      <b>{{ currentIPBX.client_name }}</b>
                    </l-popup>
                  </l-marker>
                </l-map>
              </div>
              ...
              <div class="d-flex justify-content-between align-items-center">
                <div>
                  <span class="has-text-white">
                    <b>{{guests_ipbx}} IPBX</b> à réclamer
                  </span>
                </div>
                <div>
            ...
            <div class="columns">
              <div class="column services-card" style="padding-top: 0">
                ...
                <div class="collapse active" v-show="isSearchError == false" style="overflow-y: auto; overflow-x: hidden; height: 680px">
                  <div class="collapse-trigger" v-for="s of filteredClients" :key="s.client_id + s.real_ip">
                    <div role="button" @click="setCurrentIPBX(s)" class="card-header card-ipbx"
                      ...
                      <div class="card-header-title" v-on:click="editCommonName(s)">
                        ...
                        <span style="margin-right: auto;">
                          ...
                          <small v-show="s.status === 'disconnected' && s.disconnected_since.length > 1" title="Copié dans le Presse-papier"
                            @click="copyTEXT(s.virtual_ip)">
                ...
                <div class="wrapper">
                  <div :style="!zabbix_datas['mac'] ? 'opacity: 0.4' : ''"
                    ...
                    <div class="box-container">
                      ...
                      <div class="box-content" v-show="zabbix_datas['mac'] && showLoading == false">
                        <div>
                      ...
                      <div class="box-content has-text-centered" v-show="!zabbix_datas['mac'] && showLoading == false">
                    ...
                    <div class="box-container">
                      ...
                      <div class="box-content" v-show="showLoading != true">
                        <div class="container"
                          ...
                          <span v-if="zabbix_datas['/_used']" class="status-blue">
                            {{allFormatBytes(zabbix_datas['/_used'])}}
                          </span>
                          <span v-else class="status-blue">
                            N/A
                          </span>
                          /
                  ...
                  <div :style="currentIPBX && currentIPBX.server_id <= 2 ? 'opacity: 0.4' : ''" class="box-account storage-bloc is-hoverable"
                    style="height: 150px">
                    <div class="box-container">
                      <div class="box-title">
                        Gestion de mon IPBX
                      ...
                      <div class="box-content has-text-centered" v-show="showLoading == true">
                        <div class="loading-inline" style="height: 120px; margin-top: 2%">
                          <div class="loading-icon"></div>
                          <br>
                        </div>
                      </div>
                ...
                <div class="has-mobile-cards" style="word-break: break-all;">
                  <table class="table">
                    <thead>
                      ...
                      <tbody v-for="s of filteredClients" :key="s.client_id + s.real_ip">
...
export default {
  ...
  watch: {
    filteredClients: {
      handler(newList) {
        if (newList.length > 0 && (!this.currentIPBX || !newList.some(item => item.client_id === this.currentIPBX.client_id))) {
          this.setCurrentIPBX(this.sortedClients(newList)[0]);
        }
      },
      immediate: true,
      deep: true
  ...
  computed: {
    ...
    clients() {
      return this.$store.getters.clientsWithoutPhones;
    },
    filteredClients() {
      ...
      if (this.search_badges.length > 0) {
        ...
        filtered = filtered.filter(client => {
          return s.toLowerCase().split(' ').every(v =>
            ...
            (client.real_ip && client.real_ip.toLowerCase().includes(v)) ||
            (client.virtual_ip && client.virtual_ip.toLowerCase().includes(v))
          )
        })
      }
