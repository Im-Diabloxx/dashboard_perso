<template>
  <div>
    ...
    <!--
  <div class="container is-max-widescreen"><div class="columns is-desktop mt-3"><div class="column is-three-fifths-desktop"><div class="fieldset is-fullheight"><label>
        Consommation hors-forfait de ma flotte
      </label> <div class="fieldset-content"><div class="columns mb-3"><div class="column">
            au 02/08/2024
          </div></div> <p class="has-text-right oneline status-success" style="width: 0%;">
            0,00 €
          </p> <div class="progress-wrapper status-success mb-2" style="--value: 0;"><progress max="100" class="progress is-small is-darkgrey" value="0">0</progress></div> <div class="d-flex justify-content-between"><div>
              0
            </div> <div>
              90,00 €
            </div></div></div></div></div> <div class="column is-two-fifths-desktop"><div class="fieldset"><label>
        Mes forfaits à activer
      </label> <div class="fieldset-content"><div class="d-flex justify-content-between align-items-center"><div class="pr-5"><span class="has-text-white"><b>2 forfaits</b> à activer</span> <br> <em class="help">
              La mise à disposition de votre commande peut prendre jusqu'à 30 min.
            </em></div> <button class="button is-admin-primary">
            Activer
          </button></div></div></div> <div class="blackblock sims">
      Besoin d’un forfait ?
      ...
      <div class="columns is-desktop mt-3" style="padding-right: 0.75rem">
        <div class="column is-three-fifths-desktop" style="padding: 0.75rem; padding-right: 0rem;">
          <div class="fieldset">
            ...
            <!--
            <div class="fieldset-content" style="position: relative; height: 162px;">
              <div id="ipbx-map" style="width: 100%; height: 100%;"></div>
            </div>
            -->
            ...
            <div class="columns">
              <div class="column services-card" style="padding-top: 0">
                ...
                <div class="collapse active" v-show="isSearchError == false" style="overflow-y: auto; overflow-x: hidden; height: 680px">
                  <div class="collapse-trigger" v-for="s of filteredClients" :key="s.client_id">
                    <div role="button" @click="setCurrentIPBX(s)" class="card-header card-ipbx"
                      :class="s.client_id == currentIPBX.client_id ? 'card-active' : ''" @mouseover="hoveredClient = s.client_id" @mouseleave="hoveredClient = null">
                      <div class="card-header-title" v-on:click="editCommonName(s)">
                        ...
                        <div class="icon-wrapper" :class="{ 'visible': hoveredClient === s.client_id }" style="padding-right: 1em; text-align: right;">
                          <!--
                          <a v-if="currentIPBX" title="Note" class="is-admin-secondary-light is-medium" style="opacity: 0.4">
                            <i class="material-symbols-outlined size-l">edit_note</i>
                          </a>
                          -->
                          <a v-if="currentIPBX" @click="openAddNote" title="Note" class="is-admin-secondary-light is-medium">
                            <i :class="{'yellow-icon': s.note && s.note !== ''}" class="material-symbols-outlined size-l">edit_note</i>
                          </a>
                          <a v-if="currentIPBX && s.status === 'connected'" title="Redirection vers mon IPBX" target="_blank" :href="'http://' + currentIPBX.virtual_ip" class="is-admin-secondary-light is-medium">
                            <i class="material-symbols-outlined size-l">start</i>
                          </a>
                          <a v-if="currentIPBX && s.status === 'disconnected'" @click="downloadClientVPN" title="Télécharger le certificat VPN" class="is-admin-secondary-light is-medium">
                            <i class="material-symbols-outlined size-l">download</i>
                          </a>
                          <a v-if="currentIPBX" title="Révoquer" @click="revokeClient()" class="is-admin-secondary-light is-medium">
              ...
              <div class="datas" style="margin-right: 0;">
                ...
                <div class="wrapper">
                  ...
                  <div :style="currentIPBX && currentIPBX.server_id <= 2 ? 'opacity: 0.4' : ''" class="box-account phone-status-bloc"
                    style="height: 49%; align-self: flex-end">
                    <div class="box-container">
                      <div class="box-title">
                        Gestion de mon IPBX
                        <i class="icon-hover material-symbols-outlined size-m wght-m">tune</i>
                      </div>
                      ...
                      <div class="box-content">
                        <div class="box-options">
                          <div class="list-options">
                            ...
                            <div v-if="user.id_company && user.id_company === 1" title="Données administratives" @click="goToUsers()" class="option active" style="cursor: pointer">
                              <span>
                                <i class="icon-hover material-symbols-outlined size-xxl">admin_panel_settings</i>
                              </span>
                              <span>
                                Administration
                              </span>
                            ...
                            <div title="Accèder au journal d'appel" @click="goToCallLog()" class="option active" style="cursor: pointer">
                              <span>
                                <i class="icon-hover material-symbols-outlined size-xxl">history</i>
                              </span>
                              <span>
                                Journal
                              </span>
                      ...
                      <div class="box-content">
                        <div class="box-options">
                          <div class="list-options">
                            ...
                            <div @click="revokeClient()" class="option active" style="cursor: pointer">
                              <span>
                ...
                <div class="has-mobile-cards" style="word-break: break-all;">
                  <table class="table">
                    <thead>
                      ...
                      <tbody v-for="s of filteredClients" :key="s.client_id">
    ...
    <AddNote v-if="currentIPBX" :client="currentIPBX" :active="isAddNoteModalActive ? true : false" :emitter="'modalAddNote'" @modalAddNote="noteModal" @note-updated="updateCurrentIPBXNote"></AddNote>

    <DownloadRevokeClientVPN v-if="currentIPBX" @downloadRevokeClient="getDownloadRevokeClient" :client="currentIPBX"
      :active="isDownloadRevokeClient ? true : false"></DownloadRevokeClientVPN>

    <DownloadConfigVPN v-if="currentIPBX" @downloadClient="getDownloadClient" :client="currentIPBX" :active="isDownloadClient ? true : false"></DownloadConfigVPN>
...
import ModalActivateIPBX from './ActivateIPBX.vue';
...
import AddNote from '../../modal/AddNote.vue';
...
export default {
  ...
  async mounted() {
    ...
    EventBus.on('activateIPBX', val => { this.activateIPBX = val; });
  ...
  methods: {
    ...
    initMap() {
      const defaultLatitude = 46.603354; // France
      const defaultLongitude = 1.888334;
      console.log(this.zabbixdatas)

      const latitude = this.zabbix_datas['gps_latitude'] || defaultLatitude;
      const longitude = this.zabbix_datas['gps_longitude'] || defaultLongitude;
      const map = L.map("ipbx-map").setView([latitude, longitude], 13);
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors',
      }).addTo(map);
      L.marker([latitude, longitude]).addTo(map).bindPopup("Votre IPBX est ici.").openPopup();
    },
    updateCurrentIPBXNote(newNote) {
      if (this.currentIPBX) {
        this.currentIPBX.note = newNote;
        this.$store.dispatch('updateClientNote', {
          clientId: this.currentIPBX.id,
          note: newNote,
          noteModificationDate: new Date().toISOString()
        });
      }
    },
    openAddNote() {
      this.isAddNoteModalActive = true;
    },
    noteModal(value) {
      this.isAddNoteModalActive = value;
    },
    async setCurrentIPBX(value) {
      ...
      try {
        // Utiliser Promise.all pour exécuter les appels API en parallèle
        const [dataInfos, tickets, monitorData] = await Promise.all([
          this.fetchDataInfos(header),
          this.fetchTickets(header),
          this.getMonitorFromZabbix()
        ]);

        this.datas_infos = dataInfos;
    ...
    fetchDataInfos(header) {
      let params = {
        'cn': this.currentIPBX.client_label
      };
      return this.$axios.post(process.env.ODOO_API + 'api/tickets/cn?session_id=' + this.user.session_id, { params }, { headers: header })
    ...
    fetchTickets(header) {
      ...
      return this.$axios.post(process.env.ODOO_API + 'api/tickets/test?session_id=' + this.user.session_id, { params })
        .then(response => response.data.result);
    },
    ...
    goToVPN(c) {
      this.isDownloadRevokeClient = true;
    },
    ...
    goToCallLog() {
      this.$globalState.loader = true;
      setTimeout(() => {
        this.$router.push('/products/ipbx/calllog?id=' + this.currentIPBX.client_id + '&name=' + this.currentIPBX.client_label);
      }, 1000);
    },
    goToPhonebook() {
      this.$globalState.loader = true;
      setTimeout(() => {
        this.$router.push('/products/ipbx/phonebook?id=' + this.currentIPBX.client_id + '&name=' + this.currentIPBX.client_label);
      }, 1000);
    },
    goToUsers() {
      this.$globalState.loader = true;
      setTimeout(() => {
        this.$router.push('/products/ipbx/administration?id=' + this.currentIPBX.client_id + '&name=' + this.currentIPBX.client_label);
      }, 1000);
    },
    sortedServers(server) {
      function compare(a, b) {
        if (a.client_label.toLowerCase().split(' ').join('') < b.client_label.toLowerCase().split(' ').join('')) {
          return -1;
        }
        if (a.client_label.toLowerCase().split(' ').join('') > b.client_label.toLowerCase().split(' ').join('')) {
          return 1;
    ...
    formatBytes(a, b) {
      ...
      let c = 1024,
        d = b || 4; // eslint-disable-next-line one-var
    ...
    sortedClients(clients) {
      return clients.sort((a, b) => a.client_label.toLowerCase().localeCompare(b.client_label.toLowerCase()));
    }
  },
  computed: {
    guests_ipbx() {
      return this.products_infos ? this.products_infos.guests_ipbx : '?';
    },
    clients() {
    ...
    filteredClients() {
      ...
      if (this.search_text) {
        ...
        filtered = filtered.filter(client => {
          return text.split(' ').every(v => client.client_label.toLowerCase().includes(v) || client.real_ip.toLowerCase().includes(v) || client.virtual_ip.toLowerCase().includes(v));
        });
      }

      if (this.isActif) {
        filtered = filtered.filter(client => client.status === 'connected');
      }

      if (this.isInactif) {
        filtered = filtered.filter(client => client.status === 'disconnected');
      }
