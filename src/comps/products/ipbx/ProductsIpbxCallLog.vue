<template>
    <div class="account-content" :style="tab ? 'padding-left: 250px;' : ''">
      <div class="admin-box" v-if="user.allowed_routes && user.allowed_routes.length > 0 && user.allowed_routes.includes('call_logs')" v-show="user.allowed_routes && user.allowed_routes.length > 0 && user.allowed_routes.includes('call_logs')">
...
export default {
  ...
  async created () {
    if (this.user.allowed_routes.length <= 0 || !this.user.allowed_routes.includes('call_logs')) {
      this.$router.push('/products')
    }
