<template>
          ...
          <div class="fieldset mb-5">
            ...
            <div class="fieldset-content" style="margin-bottom: 10px;">
              <div class="" style="float: right">
                <div class="field mb-0" style="min-width: 300px">
                  <TagInput :modelValue="search_contacts" />
                </div>
              </div>
              <div v-if="selectedItems.length > 0" class="d-flex" style="float: right; margin-right: 1em;">
                <div class="d-flex" :title="'Exporter les contacts'">
                  <i class="material-symbols-outlined" style="font-size: 24px; cursor: pointer;" @click="showExportContactModal = true">file_upload</i>
                </div>
                <div class="d-flex" :title="'Supprimer les contacts'">
                  <i class="material-symbols-outlined" style="font-size: 24px; cursor: pointer;" @click="deleteSelectedContacts">delete</i>
                </div>
              </div>
            </div>

            <div v-if="filteredPhonebooks.length === 0" class="fieldset-content d-flex" style="gap: 4rem; width: 100%; place-content: center;">
              <div class="has-text-centered" style="place-content: center; opacity: 0.4; padding: 30px; margin-right: 2rem;">
                <i class="material-symbols-outlined size-giga">sentiment_dissatisfied</i>
                <p>Aucune donnée n'a été récupérée</p>
              </div>
            </div>

            <div v-if="filteredPhonebooks.length > 0" class="fieldset-content d-flex" style="gap: 4rem; width: 100%">
              <div class="phonebook-list" style="width: 35%" @click.stop>
                <div class="widget-phonebook">
                  <div class="ibox-content" style="height: 680px; overflow-y: auto;">
              ...
              <div v-if="filteredPhonebooks.length > 0" class="fieldset-content d-flex" style="gap: 4rem; width: 65%; padding: 15px 20px 20px 20px;">
                <div class="view-contact-panel" style="height: 680px; overflow-y: auto; width: 100%; padding: 15px 20px 20px 20px; background: var(--section); border-radius: 50px;">
                  <transition
                    name="fade"
                  ...
                  >
                    ...
                    <div v-show="showAddContactButton" class="add-contact" style="text-align: center">
                      <div class="has-text-centered" style="opacity: 0.4; padding: 30px; margin-top: 30%; margin-right: 2rem;" @click="showCreateContactModal = true">
      ...
      <CreateContact :phonebooks="phonebooks" :defaultPhonebookId="defaultPhonebookId" :active="showCreateContactModal" @closeModal="showCreateContactModal = false" @createContact="createContact" @createMultipleContact="createMultipleContact"/>
  ...
  methods: {
    ...
    async fetchPhoneBooksAndContacts() {
      ...
      try {
        ...
        // Transform phonebooks into a map for easier access
        const phoneBookMap = {};
    ...
    */
    ...
    async createContact(contact) {
      ...
      const cn = this.$store.getters.getCn;
      contact.cn = cn
      //let contactData = this.convertContactToSend(contact)
    ...
    async createMultipleContact(contacts) {
      ...
      const contactDataArray = {
        cn: cn,
        contacts: contacts.map(contact => this.convertContactToSend(contact)) // Map contacts to the desired format
      };
    ...
    isPhonebookSelected(phonebook) {
      return phonebook.contacts.every(contact => this.selectedItems.includes(contact));
    },
