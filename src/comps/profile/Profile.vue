...
export default {
  ...
  methods: {
    ...
    async getVPN () {
      // let vpn = this.$axios.get('http://192.168.102.40:5000/download/client?id_client=' + this.user.id_vpn)
      this.$globalState.loader = true
      let vpn = await this.$axios.get(process.env.ODOO_API + 'api/vpn/download/certificate?id=' + this.user.id_vpn + '&version=2', {responseType: 'blob'})
      .then(response => {
