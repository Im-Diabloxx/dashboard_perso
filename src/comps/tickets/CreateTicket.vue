...
export default {
  ...
  computed: {
    ...
    myIPBX () {
      let cservers = this.$store.getters.clients;
      if (!cservers || !Array.isArray(cservers)) {
        return [];
      }
      ...
      if (this.ticketContext) {
        ...
        cservers = cservers.filter(s => {
          return text.toLowerCase().split(' ').every(v => 
            (s.client_label ? s.client_label.toLowerCase().includes(v) : false) || 
            (s.real_ip ? s.real_ip.toLowerCase().includes(v) : false) || 
            (s.virtual_ip ? s.virtual_ip.toLowerCase().includes(v) : false)
          );
        });
        return cservers;
      }
    ...
    myTrunk () {
      let ctrunks = this.$store.getters.trunks
      if (this.ticketTrunk) {
